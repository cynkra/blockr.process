...
returns a result. None of that is a click in your app.

The event log **is** the API. Current state is a fold over an append-only
list of events, so causing an event is the only thing an outside system
ever needs to do.
...
convention for a task that waits on the outside world: BPMN's receive task.
...
yet, a race rather than a mistake. The file stays where it is and the next
tick picks it up.
...
time is correct, because the second send of a message is ignored. That is
simpler than a watermark, and a missed run is repaired by the next one.
...
script task. First-class outbound messages are the most likely next
addition.
