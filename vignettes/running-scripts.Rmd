...
vignette: >
  %\VignetteIndexEntry{Running scripts}
...
A task whose `role` is `system` and whose `script` names a file is run by
the **worker**: a plain R process, started outside the app, that reads the
event log, runs what is ready, and appends what happened.

Nothing about it is clever, and that is the design. The board never starts
a script and holds no instance state, so closing the browser does not stop
production, two people watching see the same thing, and the worker can be
killed and restarted at any moment without losing more than the time in
between.
...
The worker ran the script, stopped at the task a person owns, and said so.
Everything it did is in the log:
...
every attempt: how long it ran, where its output went, and which version of
the code that was.
...
Exit code 0 means done; anything else means failed. Everything the script
prints is captured to `<store>/<instance>/logs/<task>-<attempt>.log`, one
file per attempt, kept.
...
A check does not report success or failure, it reports an **answer**, and
the answer routes the process. One line on stdout:
...
that will never come. Nobody wrote an if-statement; the branch is in the
table. When `fix` is done the check is re-armed and runs again, which is
the rework loop a DAG cannot express.
...
Add `sequential = TRUE` to the group row and the worker runs them one at a
time instead.

## When things go wrong

**Retries** are a column. `retry = 2` means up to three attempts; each one
is a separate log file, and the decision to retry is written into the event
log (actor `worker (retry 1/2)`) rather than hidden inside a loop.

**Timeouts** are a column too. `timeout = 600` kills the attempt after ten
minutes and records `timeout after 600s` as an `error` event.
...
what the instance is stuck on. There is no automatic rollback: the process
is a record of what happened, and what happened was a failure.

## The jobs directory is a jail

`script` is a **name inside** the jobs directory, never a path out of it.
Absolute paths, `~` and any `..` segment are refused before the filesystem
is touched, and a symlink pointing out of the directory is caught after.
Refusing is an outcome like any other — the task fails, an `error` event
carries the reason, nothing is executed.
...
process. Keep the jobs directory under version control and deploy it like
code.
...
This is usually the better shape for a deployment. The package is already in
the image, so there is no jobs directory to ship, mount or version
separately; `code_version` becomes the package version, which is a truer
answer than a hash of a folder; and the job is a documented, tested,
`R CMD check`ed function like any other.

Two things stay exactly as they are, on purpose:

- **It still runs in a subprocess** (`Rscript --vanilla -e 'mypkg::forecast()'`).
  Calling in-process would cost you the timeout (you cannot reliably
  interrupt arbitrary R or C code in the worker's own session), crash
  isolation (a segfault or an out-of-memory would take the worker with it),
  and the per-attempt log file. R startup is a fraction of a second; a real
  job is not.
- **The contract is the same.** The function takes no arguments and reads
  `PROC_ELEMENT`, `PROC_ATTEMPT`, `PROC_ARTIFACTS` and the rest from its
  environment, exactly like a script. So the two forms are interchangeable
  and a task can move from one to the other without anything else changing.

`packages` is an **allowlist, empty by default** — the same jail the jobs
directory is, for the same reason. A process definition is edited in a
browser; without the allowlist, `script` could name `base::system` and
whoever may edit a process could run anything the worker can. Only a plain
...
`--vanilla` is deliberate: a script that needs a package environment should
say so on its own first line (`renv::load()`), so what ran is visible in
the script rather than in the worker's shell.
...
scripts. A year later the log answers "what exactly ran here" without
anyone having had to remember to write it down.
...
behind by a worker that died is broken automatically. `worker_alive(store)`
answers whether one is running, which a board can display.
...
**Posit Connect** — a scheduled job (an R script, not an app) doing exactly
what the cron line does. The store has to be on a path the job can write.
...
not need the app, and the app does not need it — except that without a
worker, script tasks simply never start.
