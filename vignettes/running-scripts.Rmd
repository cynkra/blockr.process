...
A check reports an **answer** rather than a plain success or failure, and
...
that will never come. The branch is in the table, not in code. When `fix`
is done the check is re-armed and runs again, a rework loop a plain DAG
cannot express.
...
what the instance is stuck on. There is no automatic rollback; the failure
...
Refusing is an ordinary outcome: the task fails, an `error` event carries
the reason, and nothing is executed.
...
- **It still runs in a subprocess** (`Rscript --vanilla -e 'mypkg::forecast()'`).
  ...
  and the per-attempt log file. R startup takes a fraction of a second,
  negligible against any real job.
...
`packages` is an **allowlist, empty by default**, for the same reason the
jobs directory is a jail. A process definition is edited in a
