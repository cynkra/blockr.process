...
* `run_worker(packages =)` is the allowlist for that form — **empty by
  default**, because a `script` cell is edited in a browser and `base::system`
  is a function too. Only a plain `pkg::fun` is accepted: no `:::`, no
...
* **The engine/UI line is now declared and enforced.** The package is two
  ...
  semantics helpers) are marked internal — still exported, no longer
