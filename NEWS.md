...
* **The tasks block sorts by status.** A `sort` control in the toolbar,
  ...
  skipped, then done -- so unfolding a step of two thousand elements leads
  with the ones that still need somebody rather than with two hundred that
  are finished. Stable within a status, so an element keeps its place in the
  collection's own order and does not jump about as its neighbours move.
  `by id` restores the collection order.
...
* **Ingesting a batch of inbox messages no longer re-reads the instance for
  ...
  and each of them parses the whole event log, which holds the element list
  as a single JSON blob. Read once per instance per `ingest_inbox()` call
  instead of once per message: the first synchronisation of a process over
  2115 elements went from over five minutes (still unfinished) to 5.8
  seconds. Instances neither appear nor change while their own inbox is
  being ingested, so the batch can safely share one read.
